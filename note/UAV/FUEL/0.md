...
- 利用传感器数据（RayCaster）构建周围环境的详细**3D地图**（SDFMap），识别待探索的新区域（Frontier Finder），并智能规划探索方向（Heading Planner）

- 能生成*平滑且无碰撞的运动路径*（B样条轨迹、路径搜索算法），同时遵守机器人的物理限制，并通过持续优化提升效率（Bspline Optimizer）
