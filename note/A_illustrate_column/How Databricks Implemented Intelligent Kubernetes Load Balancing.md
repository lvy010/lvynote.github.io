...
由于选择仅在每个 TCP 连接时发生一次，因此相同的后端 pod 在该连接的生命周期内持续接收流量。对于短期的 HTTP/1 连接，这通常没问题。然而，对于持久的 HTTP/2 连接，结果是流量倾斜：少数 pod 过载，而其他 pod 保持空闲。
...
该架构设计为可扩展的。团队可以轻松添加新的负载均衡策略，而无需更改整个系统。例如：
