...
现在，是时候将这些部分结合起来，探讨`nanoflann`的核心引擎：**KD树索引**
...
KD树索引是`nanoflann`的核心，高效组织数据点以实现闪电般快速的最近邻搜索。

通过了解数据是静态还是动态，你可以选择合适的`nanoflann`适配器（固定数据用`KDTreeSingleIndexAdaptor`，变化数据用`KDTreeSingleIndexDynamicAdaptor`）来构建最优索引。

现在，数据已被`nanoflann`理解（通过[数据集适配器接口](01_dataset_adaptor_interface_.md)），距离被正确测量（通过[距离度量适配器](02_distance_metric_adaptors_.md)），KD树已构建，已准备好实际*获取*搜索结果！

在下一章中，我们将探讨`nanoflann`用于传递这些邻居的各种[结果集类](04_result_set_classes_.md)。
