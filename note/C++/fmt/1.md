...
sequenceDiagram
    ...
    participant Fmt函数["fmt::print / fmt::format_to / fmt::output_file"]
    participant Fmt内部["fmt 库内部"]
    participant 目标["控制台 / 内存 / 文件"]

    你的代码->>Fmt函数: 调用格式字符串和参数
    Fmt函数->>Fmt内部: 传递格式化请求
    Note over Fmt内部: 处理参数，转换为文本，处理格式规范（如对齐）。
    Fmt内部->>Fmt内部: 在内部缓冲区构建格式化文本
    Fmt内部->>目标: 将最终文本从缓冲区写入目标
    目标->>你的代码: （间接）显示或存储结果
```
