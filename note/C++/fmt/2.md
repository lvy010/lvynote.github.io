...
在[前一章：输入/输出适配器（`fmt::print`，`fmt::format_to`，`fmt::output_file`）](01_input_output_adapters___fmt__print____fmt__format_to____fmt__output_file___.md)中，我们学习了如何将精心格式化的消息传递到控制台或文件等不同目的地。但在`fmt`能够传递消息之前，它首先需要理解你提供的*原料*（你的数据）以及如何准备它们。
...
这正是`fmt`库中"参数处理"的工作。它是组织和理解你想要格式化的每一块数据的系统——无论是数字、文本还是你自己的自定义数据类型。它确保无论你给`fmt`什么，它都知道*它是什么*以及*如何将其转换为文本*。
...
sequenceDiagram
    ...
    participant Fmt函数["fmt::print / fmt::vformat"]
    participant ArgStore["fmt::dynamic_format_arg_store"]
    participant FmtFormatArgs["fmt::basic_format_args"]
    participant FmtBasicFormatArg["fmt::basic_format_arg"]
    participant Fmt内部["fmt 库内部"]

    你的代码->>ArgStore: 添加参数 (push_back)
    Note over ArgStore: 为每个参数创建单独的 basic_format_arg 对象。复制某些类型，引用其他类型。
    你的代码->>Fmt函数: 调用 ArgStore (隐式转换为 FmtFormatArgs)
    Fmt函数->>FmtFormatArgs: 按索引/名称访问参数
    FmtFormatArgs->>FmtBasicFormatArg: 返回特定的 basic_format_arg 对象
    Fmt函数->>FmtBasicFormatArg: 调用 basic_format_arg.visit(visitor)
    Note over FmtBasicFormatArg: 根据实际参数类型分派到正确的处理函数 (如 handleInt, handleString)
    FmtBasicFormatArg->>Fmt内部: 提供实际值用于格式化
    Fmt内部->>Fmt函数: 格式化文本
```

### 深入探讨：`basic_format_arg`结构

`basic_format_arg`的魔力在于它能够以标准化方式存储不同类型的数据。它通过使用`union`和枚举来跟踪实际类型实现这一点。
...
当你创建一个`basic_format_arg`（通常是间接的），`fmt`初始化`union`的正确成员并设置`type_`枚举。之后，`visit`方法使用`type_`的`switch`语句调用存储类型的正确函数重载。这样，`fmt`始终确切知道它正在处理什么类型的数据。
...
这种灵活的存储机制使`fmt::dynamic_format_arg_store`能够高效地处理混合参数类型和生命周期。
...
*   **`dynamic_format_arg_store`**：用于在运行时动态构建参数列表的"原料组织器"，支持位置和命名参数，并通过`std::cref`控制复制与引用。
