...
`D`对象是Qlib中强大、统一的数据访问入口。通过简单直观的命令，可轻松检索金融数据、查询市场日历、列出标的及计算复杂特征。掌握`D.calendar()`、`D.list_instruments()`和`D.features()`的使用，就能直接控制量化研究所需的金融信息。

了解如何使用`D`直接访问和获取数据后，下一步是学习如何为机器学习模型准备这些数据。让我们继续探索[数据处理器](03_data_handler_.md)！
