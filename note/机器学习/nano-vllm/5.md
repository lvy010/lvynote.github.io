...
现在，我们的请求已经排好队，内存也已准备就绪。那么，谁来做这些繁重的计算工作？谁负责执行那些让大型语言模型（LLM）生成文本的极其复杂的计算？
...
就像你不会直接与装配线上的机器人交谈一样，你也不会直接与模型运行器交互。它是[LLM引擎](01_llm_engine_.md)协调的内部组件。
...
CUDA图是一项强大的NVIDIA技术，用于加速重复的GPU计算。CUDA图不是每次向GPU发送单独的指令，而是记录一系列操作，然后作为一个高度优化的单元"重放"它们。这显著减少了开销，并使模型运行得更快，尤其是在逐个生成标记（解码阶段）时。
...
**模型运行器**是`nano-vllm`的动力源，是让AI模型活跃起来的专用工人。它负责将庞大的模型权重加载到GPU上，执行复杂的神经网络计算，并管理与KV缓存的交互。关键的是，它采用了CUDA图等高级优化，并支持跨多个GPU的分布式处理，以确保文本生成尽可能快速高效。

你现在已经了解了模型运行器如何获取准备好的请求并实际运行AI模型以生成新标记。但`self.model`内部是什么？在下一章中，我们将打开黑匣子，深入探讨构成大型语言模型的[神经网络层](06_neural_network_layers_.md)。
