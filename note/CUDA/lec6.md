...
Pascal + 架构下，CUDA 统一内存的按需分页因每页故障的开销，会使核函数性能变慢；对于大块数据迁移，==单步 “memcpy 式”== 操作比逐页迁移更高效

![image-20251121175804867](image-20251121175804867.png)

通过`cudaMemPrefetchAsync`显式预取内存，替代`cudaMemcpyAsync`，支持将数据定向到任意 GPU 或 CPU，从而恢复 CUDA 统一内存的性能。

![image-20251121175826944](image-20251121175826944.png)

Pascal + 架构下（详见官方文档）

可通过`cudaMemAdvise`接口给运行时显式提供内存访问行为提示（如指定只读复制、推荐存储位置、设备访问映射等），以实现细粒度性能调优，且这些提示本身不会触发数据迁移。

`cudaMemAdviseSetReadMostly`提示可让统一内存系统为只读数据在各访问处理器处生成本地副本，若某处理器写入则仅保留自身副本并使其他副本失效，且该提示会忽略设备参数。

`cudaMemAdviseSetPreferredLocation`提示可建议数据的最佳处理器存储位置，数据会按需（或预取时）迁移到该位置；若其他处理器访问，优先提供 P2P 映射，无法映射时才迁移，Volta + 还新增访问计数器辅助 GPU 决策。

> understand: 帮系统更聪明地管理内存，让程序跑得更快~

- **只读复制**：数据大多只读时，会在每个要用它的设备（CPU/GPU）存个副本；要是有设备写了，只有它的副本有效，其他都失效。
- **推荐位置**：告诉系统 “数据放哪个设备最好”，系统会按需把数据挪过去；其他设备访问时，优先走高速映射，实在不行才挪数据。
- **设备访问**：不给数据挪位置，只给指定设备开个 “快捷访问通道”，数据挪了通道也会自动更新，这样访问时就不会卡壳啦。

CUDA 统一内存（UM）主要优势是==简化编程、提升开发效率==，并非让优秀 CUDA 代码跑得更快，多数情况下==性能不如专家手动优化==的数据迁移；若误用会拖慢代码，但`能支持图遍历等设计模式`，且超量内存使用不会让任意算法都获得 GPU 级性能，适合多库内存需求场景下的轻量化优化。

---

#### 不同场景下的内存共享

- **多 GPU **： Pascal 前的显卡之间，内存分配只在当前显卡，其他显卡靠 “高速映射” 访问；Pascal 及以后，==所有显卡和 CPU 都能按需用内存==，还能通过预取 / 提示让系统更聪明地管理。
- **Power9 **： 在 Power9 机器上，CPU 和 GPU 能 “共享” 内存，==CPU 分配的内存 GPU 可以直接读，不用额外折腾==；数据要手动挪位置，现在正通过系统层面的技术让更多设备能用这功能。

可以通过跑hw来进一步理解性能上的优化，实践是检验真理的唯一标准~![image-20251121180521110](image-20251121180521110.png)

sum：
- CUDA统一内存技术（Unified Memory）通过cudaMallocManaged实现了CPU与GPU内存的统一管理，简化了异构编程中的内存操作。

该技术具有以下特点：
- 1）支持超量内存分配，允许GPU访问超过显存容量的数据；
- 2）自动按需迁移内存页，优化数据访问效率；
- 3）Pascal+架构支持系统级原子操作和细粒度内存管理。

开发者可通过cudaMemAdvise和cudaMemPrefetchAsync等API进行性能调优。该技术==特别适合处理链表、图结构等复杂数据，使深拷贝操作变得简单高效==，大幅提升了异构计算的开发效率。