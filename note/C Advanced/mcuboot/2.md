...
```
           ┌──────────────────────────────────────────────┐
           │                image_header                  │
           │  魔数 | 运行地址 | 头大小 | 受保护TLV大小 | ... │
           └──────────────────────────────────────────────┘
           │← 固定长度，MCUboot first scan               │
           ▼
┌─────────────────────────────────────────────────────────────┐
│                   应用程序代码 (ih_img_size)                 │
│      真正要运行的固件指令，大小由 header 告诉 MCUboot       │
└─────────────────────────────────────────────────────────────┘
           ▼
┌─────────────────────────────────────────────────────────────┐
│          TLV 区域 (Type–Length–Value)                       │
│  ┌──────────────┬──────────────┬──────────────┬───────────┐ │
│  │ 受保护 TLV   │←ih_protect_* │ 非受保护 TLV │ 其他元数据 │ │
│  │ 参与签名验证 │              │ 方便功能扩展 │           │ │
│  └──────────────┴──────────────┴──────────────┴───────────┘ │
└─────────────────────────────────────────────────────────────┘
```
...
![image-20251201213908622](image-20251201213908622.png)

想像 MCUboot 先看身份证：魔数正确才继续；再根据 `ih_img_size` 知道需要从闪存读取多少代码；`ih_flags` 告诉它要不要解密/解压；`ih_ver` 用来比对是不是新版本。

### TLV 区域 ＝ 附在盒子上的标签

![image-20251201214222266](image-20251201214222266.png)

常见类型：
- `IMAGE_TLV_SHA256`：受保护 TLV 中的哈希，用来确认代码没有被改。
- `IMAGE_TLV_ECDSA_SIG`：数字签名，验证固件来自可信源。
- `IMAGE_TLV_SEC_CNT`：安全计数器，阻止回滚旧版本。
- `IMAGE_TLV_DEPENDENCY`：声明依赖关系（例如需要的 bootloader 版本）。
- `IMAGE_TLV_ENC_AES256` / `IMAGE_TLV_DECOMP_SIZE`：提供解密密钥或解压后大小，帮助处理加密/压缩固件。

这样，可以把整个固件映像想成“贴满标签的安全包裹”。==MCUboot 拆包前会依次扫==身份证、检查封条（哈希/签名）、查看附注（TLV），==全部安全后才把真正的应用代码交给 MCU 运行==。
