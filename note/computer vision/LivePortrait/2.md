...
sequenceDiagram
    ...
    Note over LivePortraitPipeline: 步骤1：加载输入
    LivePortraitPipeline->>Cropper: crop_source_image/video(源数据)
    Cropper-->>LivePortraitPipeline: 裁剪后源数据, 源关键点
    LivePortraitPipeline->>Cropper: crop_driving_video(驱动数据)
    Cropper-->>LivePortraitPipeline: 裁剪后驱动数据, 驱动关键点
    Note over LivePortraitPipeline: 步骤2：创建动作模板
    LivePortraitPipeline->>LivePortraitWrapper: prepare_source(裁剪后源)
    LivePortraitWrapper-->>LivePortraitPipeline: 源特征, x_s(基础关键点)
    LivePortraitPipeline->>LivePortraitWrapper: get_kp_info(裁剪后驱动帧)
    LivePortraitWrapper-->>LivePortraitPipeline: 驱动动作信息
    Note over LivePortraitPipeline: 步骤3：逐帧动画生成
    loop 每帧循环
        LivePortraitPipeline->>LivePortraitWrapper: warp_decode(源特征, x_s, 当前帧驱动动作)
        LivePortraitWrapper-->>LivePortraitPipeline: 动画帧数据
        LivePortraitPipeline->>输出视频: 添加帧
    end
    LivePortraitPipeline->>输出视频: 最终化视频（添加音频，保存）
    输出视频-->>LivePortraitPipeline: 视频路径
    LivePortraitPipeline-->>GradioPipeline: 动画视频路径
```
