链接：

# docs：Fluss分布式数据系统

Fluss是一个专为**实时数据处理与存储**设计的分布式数据系统

提供**客户端API**供应用程序交互数据，由中央*协调服务器(Coordinator Server)*管理，并通过*分片服务器(Tablet Server)*分布式存储数据副本。Fluss支持*数据操作*如扫描、写入和记录查找，并提供与Iceberg、Flink等*数据湖生态集成*能力。

## 系统架构

```mermaid
flowchart TD
    A0["Fluss连接与客户端API
"]
    A1["表数据操作(扫描/写入/查找)
"]
    A2["协调服务器
"]
    A3["分片服务器与副本
"]
    A4["元数据管理(基于ZooKeeper)
"]
    A5["数据湖集成
"]
    A6["Flink连接器
"]
    A7["客户端写入管理
"]
    A0 -- "提供API支持" --> A1
    A0 -- "从元数据系统查询" --> A4
    A0 -- "封装客户端功能" --> A7
    A1 -- "依赖" --> A7
    A2 -- "管理" --> A3
    A2 -- "持久化元数据" --> A4
    A2 -- "协调数据分层" --> A5
    A3 -- "服务" --> A1
    A3 -- "缓存元数据" --> A4
    A3 -- "分层存储数据" --> A5
    A4 -- "存储配置" --> A2
    A4 -- "提供配置" --> A0
    A5 -- "提供Flink目录" --> A6
    A6 -- "集成到" --> A0
    A6 -- "实现Flink操作" --> A1
    A6 -- "委托写入操作" --> A7
    A7 -- "申请分区" --> A2
    A7 -- "批量发送数据" --> A3
```

## 章节

1. [Fluss连接与客户端API](01_fluss_connection___client_api_.md)
2. [表数据操作(扫描/写入/查找)](02_table_data_operations__scan__write__lookup__.md)
3. [客户端写入管理](03_client_side_write_management_.md)
4. [Flink连接器](04_flink_connector_.md)
5. [协调服务器](05_coordinator_server_.md)
6. [分片服务器与副本](06_tablet_server___replicas_.md)
7. [元数据管理(基于ZooKeeper)](07_metadata_management__zookeeper_backed__.md)
8. [数据湖集成](08_data_lake_integration_.md)

